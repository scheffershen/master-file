...
class AddEuqppvEntityFieldSubscriber implements EventSubscriberInterface
{
    private $em;
    ...
    {
        return [FormEvents::PRE_SET_DATA => 'onEuqppvEntitySelected'];
    }

    public function onEuqppvEntitySelected(FormEvent $event): void
    {
        $form = $event->getForm();
        $data = $event->getData();
        $euqppvEntity = $data->getEuqppvEntity();
        ...
        if ($euqppvEntity) {
            $form->add('euQPPV', EntityType::class, [
                        'label' => 'psmf.euQPPV',
